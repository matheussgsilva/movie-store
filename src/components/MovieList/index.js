...
const MovieList = ({ moviesId, sessionTitle }) => {
    ...
    useEffect(() => {
        const URL = `https://api.themoviedb.org/3/discover/movie?api_key=5e0ac166209d17595d2a5c230da806c7&language=pt-BR&sort_by=popularity.desc&include_adult=false&include_video=false&page=${currentPage}&with_genres=${moviesId}&with_watch_monetization_types=flatrate`
        fetch(URL)
        .then(res => res.json())
